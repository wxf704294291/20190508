...
function get_software_list()
{
	...
	if ($result === false) {
		$filter = array();
		$filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'software_id' : trim($_REQUEST['sort_by']);
		$filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
		$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('software_download');
		$filter['record_count'] = $GLOBALS['db']->getOne($sql);
		$filter = page_and_size($filter);
		$sql = 'SELECT software_id, software_name,software_version,software_size,create_time,software_url,description,sort_number' . ' FROM ' . $GLOBALS['ecs']->table('software_download') . ' ORDER by ' . $filter['sort_by'] . ' ' . $filter['sort_order'];
	...
	return array('filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
}
...
if ($_REQUEST['act'] == 'list') {
	$smarty->assign('ur_here', $_LANG['software_list_title']);
	$smarty->assign('action_link', array('text' => $_LANG['add_software'], 'href' => 'software_download.php?act=add'));
	$smarty->assign('full_page', 1);
	$software_list = get_software_list();
	$smarty->assign('software_list', $software_list['list']);
	$smarty->assign('filter', $software_list['filter']);
	$smarty->assign('record_count', $software_list['record_count']);
	$smarty->assign('page_count', $software_list['page_count']);
	$sort_flag = sort_flag($software_list['filter']);
	...
	$sort_flag = sort_flag($software_list['filter']);
	$smarty->assign($sort_flag['tag'], $sort_flag['img']);
	make_json_result($smarty->fetch('software_download.dwt'), '', array('filter' => $software_list['filter'], 'page_count' => $software_list['page_count']));
}
